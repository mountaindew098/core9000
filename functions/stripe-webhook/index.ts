...
// ... (imports and main Deno.serve block are unchanged) ...
async function persistInvoiceToDatabase(invoice, stripe, supabase) {
  try {
    ...
    if (stripeCustomerId) {
      // 1. Look up user and subscription ID using Stripe Customer ID
      const { data: subs, error: subsErr } = await supabase.from('subscriptions').select('id, user_id, restaurant_id') // Added restaurant_id for the next step
      .eq('stripe_customer_id', stripeCustomerId).limit(1).maybeSingle();
      if (subsErr) {
        throw new Error(`DB Error during subscription lookup: ${subsErr.message}`);
      }
      if (subs) {
        // Found the user and subscription link
        userId = subs.user_id || null;
        subscriptionId = subs.id || null;
        // 2. Fetch the restaurant name using the found user ID or restaurant ID
        if (userId) {
          const { data: profileData } = await supabase.from('restaurants').select('name').eq('owner_id', userId) // Assuming owner_id is the link to user_id
          .limit(1).maybeSingle();
          if (profileData?.name) {
    ...
    // ⭐ CRITICAL FIX: Ensure userId is not null before proceeding
    if (!userId) {
      // If we cannot link the invoice to a user, we must fail loudly.
      const errorMsg = `CRITICAL: Could not find user_id for Stripe Customer ID: ${stripeCustomerId}. Invoice not saved.`;
      console.error(errorMsg);
      // Throw an error so the webhook is marked as failed and Stripe retries.
    ...
    const discount = invoice.discount?.amount ?? invoice.total_discount_amounts?.reduce((sum, d)=>sum + d.amount, 0) ?? 0;
    ...
    console.log(`Invoice ${invoice.id} persisted successfully (user: ${userId})`);
  } catch (error) {
